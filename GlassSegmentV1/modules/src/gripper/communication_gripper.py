...
class GripperSerial():
    ...
    def set(self, command):
        self.set_gripper(*self.unpickle(command))
        response = self.read_bytes(8)
    ...
    def add_CRC(self, cmd_init):
        crc = self.calculate_CRC(cmd_init)
        return cmd_init + crc

    def unpickle(self, cmd):
        cmds_str = cmd.split(';')
